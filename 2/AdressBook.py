...
class Record:
    def __init__(self, name: Name, phones: Phone =[], mails: Mail =[], adress: Adress =[], birthday: Birthday = None) -> None:
        self.name = name
    ...
    def __repr__(self):
        return f'User {self.name.value} - Phones: {str(self.phone_list)}' \
...
def add(contacts, *args):
    ...
    if name.value in contacts:
        ...
        writing_db(contacts)
        return f'Add phone number: {phone.value} for {name.value}'
    else:
        contacts[name.value] = Record(name, [phone], [], [], birthday)
        writing_db(contacts)
        return f'Add {name.value}: {phone.value}'
